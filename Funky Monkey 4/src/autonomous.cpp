...
void Auton::rightMiddle(){
  ...
  double mogoY = 0;
  base.driveToPoint(1, 21, 1, true, 20, 127);
  base.turnToPoint(-36, 40);
  base.driveToMogo(-36, 40, false, 46, 50, 0.5, false, 20, 127);
  fourBar.claw.close();
  base.driveToPoint(1, 21, 1, true, 20, 127);
  fourBar.setState(LiftTargets::hover);
  pros::delay(300);
  ...
  twoBar.close();
  pros::delay(300);
  base.turnToPoint(mogoX, mogoX);
  base.driveToMogo(mogoX, mogoY, false, mogoX - 10, mogoY + 10, 0.5, false, 20, 127);
  fourBar.claw.close();
  fourBar.setState(LiftTargets::score);
  rollers.setState(RollersState::smart);
  base.turnToAngle(0);
